...
import cn.lili.modules.file.serviceimpl.FileServiceImpl;
...
//    private FileManagerPlugin fileManagerPlugin;
    ...
    public ResultMessage<Object> upload(MultipartFile file,
        ...
        try {
            FileManagerPlugin fileManagerPlugin = fileService.getFileManagerPlugin();
            OssSetting ossSetting = fileService.getOssSetting();

            if(ossSetting.getOssType() == OssType.ALI.value()){
                fileManagerPlugin.pathUpload(dir, fileKey);
            }
            fileKey = dir + fileKey;
            InputStream inputStream = file.getInputStream();
            //上传至第三方云服务或服务器
            result = fileManagerPlugin.inputStreamUpload(inputStream, fileKey, file.getContentType());
            //保存数据信息至数据库
